...
module.exports = function (passport) {
  ...
  function (req, email, password, done) { // callback with email and password from our form
    ...
    User.findOne({ 'mail': email }, function (err, user) {
      ...
      if (!user) { return done(null, false, req.flash('error', 'Sorry Your Account Not Exits ,Please Create Account.')) } // req.flash is the way to set flashdata using connect-flash

      // if the user is found but the password is wrong
      if (!user.validPassword(password)) { return done(null, false, req.flash('error', 'Email and Password Does Not Match.')) } // create the loginMessage and save it to session as flashdata

      if (user.status === 'inactive') { return done(null, false, req.flash('error', 'Your Account Not Activated ,Please Check Your Email')) } // create the loginMessage and save it to session as flashdata
